...
def main(**kwargs):
    ...
    # 准备数据集
    dataset_train = get_preprocessed_dataset2(tokenizer, train_config.dataset, 'train', train_config.on_policy).shuffle(seed=42)
    accelerator.print(f"--> Training Set Length = {len(dataset_train)}")

    dataset_val = get_preprocessed_dataset2(tokenizer, train_config.dataset, 'val', train_config.on_policy)
