...
from llama_recipes.utils.dataset_utils import get_dataset_implicit
...
def main(**kwargs):
    ...
    dataset_test = get_dataset_implicit(tokenizer, 'test', train_config)

    if train_config.test_original_model:
        accelerator.print("==============original test================")
        test_implicit(train_config, dataset_test, tokenizer, wandb_run, original=True)

    if True:
        print("==============finetuned test2stage================")
        test_ece, test_auroc, test_acc2 = test_implicit(train_config, dataset_test, tokenizer, wandb_run, original=False)
